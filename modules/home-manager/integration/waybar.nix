...
let
  waybarDir = ../waybar;
  rf = name: builtins.readFile (waybarDir + ("/" + name));

  themeCSS = theme.overrides.waybar or
    (import ../templates/waybar.nix { colors = theme.colors; });
in
{
  xdg.configFile = {
    "waybar/config.jsonc".text = rf "config.jsonc";
...
.left #custom-nyx, .right #custom-nyx {
  margin: 1.5px 0;
  padding: 6px 0;
  min-width: 0;
  min-height: 12px;
